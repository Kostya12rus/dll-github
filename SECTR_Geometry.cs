...
public static class SECTR_Geometry
{
  ...
  public static Bounds ComputeBounds(Terrain terrain)
  {
    if (!(bool) ((Object) terrain))
      return new Bounds();
    Vector3 size = !((Object) terrain.terrainData != (Object) null) ? Vector3.zero : terrain.terrainData.size;
  ...
  public static bool FrustumIntersectsBounds(Bounds bounds, List<Plane> frustum, int inMask, out int outMask)
  {
    ...
    while (num <= inMask)
    {
      if ((num & inMask) != 0)
      {
        ...
        if ((double) ((float) ((double) center.x * (double) plane.normal.x + (double) center.y * (double) plane.normal.y + (double) center.z * (double) plane.normal.z) + plane.distance) + ((double) extents.x * (double) Mathf.Abs(plane.normal.x) + (double) extents.y * (double) Mathf.Abs(plane.normal.y) + (double) extents.z * (double) Mathf.Abs(plane.normal.z)) < 0.0)
          return false;
        outMask |= num;
      }
      ++index;
