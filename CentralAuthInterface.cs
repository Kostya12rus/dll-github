...
public class CentralAuthInterface : ICentralAuth
{
  ...
  {
    Console.singleton.AddLog("Authentication token obtained from central server.", (Color32) Color.green, false);
    this._s.CallCmdSendToken(token);
  }

  public void RequestBadge(string token)
  {
    this._s.GetComponent<ServerRoles>().RequestBadge(token);
  }
  ...
  public void Fail()
  {
    ...
    {
      Console.singleton.AddLog("Failed to obtain authentication token from central server.", (Color32) Color.red, false);
  ...
  public void Ok(string steamId, string nickname, string ban, string steamban, string server, bool bypass)
  {
    ...
    this._s.CallTargetConsolePrint(this._s.connectionToClient, "Accepted your authentication token (your steam id " + steamId + ") with global ban status " + ban + " signed by " + server + " server.", "green");
    if ((!bypass || !ServerStatic.GetPermissionsHandler().IsVerified) && BanHandler.QueryBan(steamId, (string) null).Key != null)
    {
      this._s.CallTargetConsolePrint(this._s.connectionToClient, "You are banned from this server.", "red");
      ServerConsole.AddLog("Player kicked due to local SteamID ban.");
      ServerConsole.Disconnect(this._s.connectionToClient, "You are banned from this server.");
    }
    else if ((!bypass || !ServerStatic.GetPermissionsHandler().IsVerified) && !WhiteList.IsWhitelisted(steamId))
    {
      this._s.CallTargetConsolePrint(this._s.connectionToClient, "You are not on the whitelist!", "red");
      ServerConsole.AddLog("Player kicked due to whitelist enabled.");
      ServerConsole.Disconnect(this._s.connectionToClient, "You are not on the whitelist for this server.");
    }
    else if ((ConfigFile.ServerConfig.GetBool("use_vac", true) || ServerStatic.PermissionsHandler.IsVerified) && steamban != "0")
    {
      this._s.CallTargetConsolePrint(this._s.connectionToClient, "You have active steam ban (" + steamban + " ban).", "red");
      ServerConsole.AddLog("Player kicked due to steam ban (" + steamban + " ban).");
      ServerConsole.Disconnect(this._s.connectionToClient, "You have active steam ban (" + steamban + " ban).");
    }
    else if ((ConfigFile.ServerConfig.GetBool("global_bans_cheating", true) || ServerStatic.PermissionsHandler.IsVerified) && ban == "1")
    {
      this._s.CallTargetConsolePrint(this._s.connectionToClient, "You have been globally banned for cheating.", "red");
      ServerConsole.AddLog("Player kicked due to global ban for cheating.");
      ServerConsole.Disconnect(this._s.connectionToClient, "You have been globally banned for cheating.");
    }
    else if ((ConfigFile.ServerConfig.GetBool("global_bans_griefing", true) || ServerStatic.PermissionsHandler.IsVerified) && ban == "2")
    {
      this._s.CallTargetConsolePrint(this._s.connectionToClient, "You have been globally banned for griefing.", "red");
      ServerConsole.AddLog("Player kicked due to global ban for griefing.");
      ServerConsole.Disconnect(this._s.connectionToClient, "You have been globally banned for griefing.");
    }
    else
    {
      ...
      if (component.BypassStaff)
        component.GetComponent<CharacterClassManager>().CallTargetConsolePrint(this._s.connectionToClient, "Your have the ban bypass flag, so you can't be banned from this server.", "cyan");
  ...
  {
    this._s.CallTargetConsolePrint(this._s.connectionToClient, "Your authentication token is invalid - " + reason + ".", "red");
