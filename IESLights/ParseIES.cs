...
namespace IESLights
{
  public static class ParseIES
  {
    ...
    public static IESData Parse(string path, NormalizationMode normalizationMode)
    {
      ...
      IESData iesData = new IESData() { VerticalAngles = floatList1, HorizontalAngles = floatList2, CandelaValues = floatListList, PhotometricType = photometricType };
    ...
    private static void PadHorizontal(IESData iesData, int longestSide)
    {
      ...
      int num2 = num1 / 2;
      IESData iesData1 = iesData;
      int num3 = num2;
      iesData.PadAfterAmount = num3;
      int num4 = num3;
      iesData1.PadBeforeAmount = num4;
    ...
    private static float CalculateHalfSpotFov(IESData iesData)
    {
      ...
      if (iesData.VerticalType == VerticalType.Top || iesData.VerticalType == VerticalType.Bottom && (double) iesData.VerticalAngles[0] == -90.0)
    ...
    private static float CalculateHalfSpotlightFovForBottomHalf(IESData iesData)
    {
      for (int index1 = iesData.VerticalAngles.Count - 1; index1 >= 0; --index1)
      {
        for (int index2 = 0; index2 < iesData.NormalizedValues.Count; ++index2)
        {
          ...
          {
            if (index1 < iesData.VerticalAngles.Count - 1)
              return iesData.VerticalAngles[index1 + 1];
            return iesData.VerticalAngles[index1];
          }
        }
      }
      return 0.0f;
      ...
      for (int index1 = 0; index1 < iesData.VerticalAngles.Count; ++index1)
      {
        for (int index2 = 0; index2 < iesData.NormalizedValues.Count; ++index2)
        {
          if ((double) iesData.NormalizedValues[index2][index1] >= 0.100000001490116)
          {
            ...
            {
              if (index1 > 0)
                return 180f - iesData.VerticalAngles[index1 - 1];
              return 180f - iesData.VerticalAngles[index1];
            }
            if (index1 > 0)
              return -iesData.VerticalAngles[index1 - 1];
            return -iesData.VerticalAngles[index1];
      ...
      foreach (List<float> candelaValue in iesData.CandelaValues)
      {
        ...
        if (squashHistogram)
        {
          for (int index = 0; index < candelaValue.Count; ++index)
            floatList1.Add(Mathf.Log(candelaValue[index]));
        }
        else
          floatList1.AddRange((IEnumerable<float>) candelaValue);
        for (int index1 = 0; index1 < candelaValue.Count; ++index1)
        {
